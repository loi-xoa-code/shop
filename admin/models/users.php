...
function changePassword($id, $newpassword, $currentPassword)
{
    ...
    if ($rows <> 1) {
        ...
        try {
            ...
            $mail->Subject = 'You have Change your Password | Blue Electronics | By Tân Hồng IT';
            $mail->Body = $htmlStr;
    ...
    if (mysqli_num_rows(mysqli_query($linkConnectDB, "SELECT user_email FROM users WHERE user_email='$email_check'")) != 0 && mysqli_num_rows(mysqli_query($linkConnectDB, "SELECT user_email FROM users WHERE id='$id_check' AND user_email='$email_check'")) <> 1) {
        ...
        if ($user_edited['user_email'] != $currentEmail) {
            ...
            try {
                ...
                $verificationLink = "https://blue-electronics.000.pe/index.php?controller=register&action=reactivate&code=" . $verificationCode;
                //content
                $htmlStr = "";
                $htmlStr .= "Xin chào " . $user_edited['user_name'] . " (" . $user_edited['user_username']  . "),<br /><br />";
                $htmlStr .= "Bạn vừa đổi email mới cho tài khoản của bạn? Vui lòng nhấp vào nút bên dưới để xác minh đổi email của bạn và có quyền truy cập vào trang quản trị của Blue Electronics.<br /><br /><br />";
                $htmlStr .= "<a href='{$verificationLink}' target='_blank' style='padding:1em; font-weight:bold; background-color:blue; color:#fff;'>VERIFY EMAIL</a><br /><br /><br />";
                $htmlStr .= "Cảm ơn bạn đã tham gia cùng website bán hàng của Blue Electronics.<br><br>";
                $htmlStr .= "Trân trọng,<br />";
                $htmlStr .= "Blue Electronics<br />";
                //Server settings
                $mail->CharSet = "UTF-8";
                ...
                //$mail->addBCC('BCCemail@gmail.com');
                $mail->isHTML(true); // Set email format to HTML
                $mail->Subject = 'Verification New Email | Blue Electronics | Change Email | By Tân Hồng IT';
                $mail->Body = $htmlStr;
                $mail->AltBody = $htmlStr; //None HTML
                $result = $mail->send();
                if (!$result) {
...
function user_add()
{
    ...
    if (mysqli_num_rows(mysqli_query($linkConnectDB, "SELECT user_username FROM users WHERE user_username='$username'")) > 0) {
        ...
        try {
            ...
            //$mail->addBCC('BCCemail@gmail.com');
            $mail->isHTML(true); // Set email format to HTML
            $mail->Subject = 'Verification Users | Blue Electronics | Subscription | By Tân Hồng IT';
            $mail->Body = $htmlStr;
            $mail->AltBody = $htmlStr; //None HTML
            $result = $mail->send();
            if (!$result) {
